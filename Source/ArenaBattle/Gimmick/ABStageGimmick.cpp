...
AABStageGimmick::AABStageGimmick()
{
	...
	for (FName GateSocket : GateSockets)
	{
		...
		Gate->SetupAttachment(Stage, GateSocket); // 스태틱 메시에 설정된 소켓에다 부착 시킨다.
...
void AABStageGimmick::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	if (PropertyName == GET_MEMBER_NAME_CHECKED(AABStageGimmick, CurrentState))
	{
		UE_LOG(LogTemp, Warning, TEXT("CurrentState changed to: %s"), *UEnum::GetValueAsString(CurrentState));
		SetState(CurrentState);
	}
}
#endif
...
void AABStageGimmick::SetState(EStageState InNewState)
{
	...
	//	StateChangeActions[CurrentState].StageDelegate.Execute();
	//}
...
void AABStageGimmick::OnOpponentDestoryed(AActor* DestoryedActor)
{
...
void AABStageGimmick::SpawnRewardBoxes()
{
	for (const auto& RewardBoxLocation : RewardBoxLocations)
	{
		...
		AABItemBox* RewardBoxActor = GetWorld()->SpawnActorDeferred<AABItemBox>(AABItemBox::StaticClass(), SpawnTransform);
		if (RewardBoxActor)
		{
